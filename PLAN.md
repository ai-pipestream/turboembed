...
by field) and the tokenizer. The Swift package (`bindings/swift`, module
`PipestreamTurbo`) is in, with the same cases as an executable runner
passing on `krickert-mac` (Apple M2), and its generation and tokenizer
wrappers mirror the Java ones. Not yet: the push form `turbo_generate` and
the chunk planner in the bindings, and the timings.
