...
`krick` under `--illegal-native-access=deny`. The Swift package
(`bindings/swift`, module `PipestreamTurbo`) is in, with the same cases as
an executable runner passing on `krickert-mac` (Apple M2). Not yet: the
`krick-1` run, generation and tokenizer wrappers in both bindings, and the
timings.
