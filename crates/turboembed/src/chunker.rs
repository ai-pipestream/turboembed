...
//!
//! This is the optional chunking utility described in the repository's
//! `docs/library-design.md` ("Tokenization, chunking, and future analysis")
//! and roadmap M6. It prepares model-sized text chunks for the embedding
//! path; callers can always skip it and provide their own chunks or prepared
//! tokens. A usage guide lives in `docs/chunking.md`.
//!
//! Contract:
//!
...
//!
//! This utility is host CPU code with no GPU or C-ABI surface. Exposing it
//! through the frozen C ABI or the Java adapters (including UTF-16 offset
//! conversion) is a separate, explicitly versioned extension.
...
fn pack_units(
    ...
    let fits = |i: usize, j: usize| -> bool {
        counter.count_tokens(&text[units[i].start..units[j].end]) <= budget
    };
    let mut i = 0usize;
    ...
    while i < units.len() {
        ...
        if config.overlap_tokens > 0 {
            while next > i + 1 {
                let cand = next - 1;
                let overlap_ok = counter.count_tokens(&text[units[cand].start..units[j].end])
                    <= config.overlap_tokens;
