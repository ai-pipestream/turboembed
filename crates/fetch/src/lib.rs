...
/// One file in a prepared-SDK source directory: (HF path, dest filename).
#[derive(Debug, Clone, Copy)]
...
/// Prepared native SDK bundle sources (see docs/native-sdk.md). These are the
/// upstream files that `scripts/prepare-native-bundle.py` converts into a
/// verified bundle. The revision and license are qualification decisions
/// pinned in source (like [`RERANK_REVISION`]), never floating branches.
/// Dest is `models/prepared-src/<alias>/`.
#[derive(Debug, Clone, Copy)]
pub struct PreparedSource {
    ...
    pub dest: &'static str,
    pub files: &'static [PreparedSourceFile],
}

pub const PREPARED_SOURCES: &[PreparedSource] = &[PreparedSource {
    alias: "minilm",
    repo: "sentence-transformers/all-MiniLM-L6-v2",
    revision: "1110a243fdf4706b3f48f1d95db1a4f5529b4d41",
    license: "Apache-2.0",
    dest: "models/prepared-src/minilm",
    files: &[
        PreparedSourceFile {
...
/// Raspberry Pi AI HAT+ (Hailo-8/8L; Hailo-10H later) embedding artifacts for
/// TurboEmbed `Device::Hailo`. Dest is `models/hailo/<alias>/`.
///
/// Two artifact classes per alias:
///   * `hf_files` — tokenizer files from the pinned HF revision (same pin as
///     the ONNX/prepared lanes).
///   * `hef_files` — compiled encoder HEFs. The official Hailo Model Zoo
///     publishes all-MiniLM-L6-v2 HEFs for hailo8 and hailo8l (the HEF is
///     arch-locked); these carry absolute S3 URLs in the manifest. A
///     hailo10h HEF needs a local DFC 5.x compile (docs/hailo-embed.md).
///
/// `config.json` + `embedding_tables.bin` are generated locally by
/// scripts/export-minilm-hailo-tables.py (not downloadable) and `model.hef`
/// is selected per board by scripts/hailo-select-hef.sh — see the runbook.
#[derive(Debug, Clone, Copy)]
pub struct HailoSource {
    pub alias: &'static str,
    pub repo: &'static str,
    pub revision: &'static str,
    pub dest: &'static str,
    pub hf_files: &'static [&'static str],
    pub hef_files: &'static [(&'static str, &'static str)],
}

pub const HAILO_SOURCES: &[HailoSource] = &[HailoSource {
    alias: "minilm",
    repo: "sentence-transformers/all-MiniLM-L6-v2",
    revision: "1110a243fdf4706b3f48f1d95db1a4f5529b4d41",
    dest: "models/hailo/minilm",
    hf_files: &["tokenizer.json", "vocab.txt"],
    hef_files: &[
        (
            "model.hailo8l.hef",
            "https://hailo-model-zoo.s3.eu-west-2.amazonaws.com/ModelZoo/Compiled/v2.19.0/hailo8l/all_minilm_l6_v2.hef",
        ),
        (
            "model.hailo8.hef",
            "https://hailo-model-zoo.s3.eu-west-2.amazonaws.com/ModelZoo/Compiled/v2.19.0/hailo8/all_minilm_l6_v2.hef",
        ),
        (
            "model.hailo8l.v2a.hef",
            "https://hailo-model-zoo.s3.eu-west-2.amazonaws.com/ModelZoo/Compiled/v2.19.0/hailo8l/all_minilm_l6_v2_v2a.hef",
        ),
        (
            "model.hailo8.v2a.hef",
            "https://hailo-model-zoo.s3.eu-west-2.amazonaws.com/ModelZoo/Compiled/v2.19.0/hailo8/all_minilm_l6_v2_v2a.hef",
        ),
    ],
}];
...
impl ManifestKind {
    pub fn manifest_file(self) -> &'static str {
        match self {
            ...
            Self::PreparedSources => "prepared-sources.json",
            Self::Hailo => "hailo-embeddings.json",
        }
    }
    ...
    pub fn as_str(self) -> &'static str {
        match self {
            ...
            Self::PreparedSources => "prepared-sources",
            Self::Hailo => "hailo",
        }
    }
}
...
pub fn prepared_source(alias: &str) -> Option<&'static PreparedSource> {
    PREPARED_SOURCES.iter().find(|s| s.alias == alias)
}

pub fn prepared_known_aliases() -> BTreeMap<String, String> {
    PREPARED_SOURCES
        .iter()
        .map(|s| (s.alias.to_string(), s.repo.to_string()))
        .collect()
...
pub fn hailo_source(alias: &str) -> Option<&'static HailoSource> {
    HAILO_SOURCES.iter().find(|s| s.alias == alias)
}

pub fn hailo_known_aliases() -> BTreeMap<String, String> {
    HAILO_SOURCES
        .iter()
        .map(|s| (s.alias.to_string(), s.repo.to_string()))
        .collect()
...
const HAILO_COMMENT: &str = "SHA-256 manifest for Raspberry Pi AI HAT+ (Hailo) embedding artifacts. Generated by cargo run -p inferstream-fetch -- --hailo --update-manifest; do not edit hashes by hand. Tokenizer files come from the pinned HF revision; model.hailo8l.hef / model.hailo8.hef are the official Hailo Model Zoo v2.19.0 artifacts (arch-locked). config.json and embedding_tables.bin are generated locally by scripts/export-minilm-hailo-tables.py; model.hef is selected per board with scripts/hailo-select-hef.sh. See docs/hailo-embed.md.";

pub fn cmd_update_hailo_manifest(
    aliases: &[String],
    manifest_path: &Path,
    root: &Path,
    store: bool,
    ...
    let mut manifest = if manifest_path.exists() {
        ...
        Manifest {
            schema_version: 1,
            comment: Some(HAILO_COMMENT.to_string()),
            models: BTreeMap::new(),
            mlx_repos: BTreeMap::new(),
        }
    };
    ...
    for alias in aliases {
        let spec = hailo_source(alias)
            .ok_or_else(|| FetchError::msg(format!("{alias}: not in HAILO_SOURCES")))?;
        let revision = spec.revision.to_string();
        writeln!(
            out,
            "--- pinning {alias}  <-  {}@{} + Hailo Model Zoo HEFs ---",
            spec.repo, revision
        )?;
        let dest = spec.dest.to_string();
        let mut files = Vec::new();
        ...
        for (dest_name, url) in spec.hef_files {
            let target = if store {
                Some(root.join(&dest).join(dest_name))
            } else {
                None
            };
            writeln!(out, "  hashing {dest_name} (Hailo Model Zoo S3) ...")?;
            let _ = out.flush();
            let (digest, size) = match stream_download(url, target.as_deref()) {
                Ok(v) => v,
                Err(e) => {
...
const PREPARED_COMMENT: &str = "SHA-256 manifest for TurboEmbed prepared native SDK bundle sources. Generated by cargo run -p inferstream-fetch -- --prepared --update-manifest; do not edit hashes by hand. The revision and license are the qualified pins recorded in PREPARED_SOURCES (crates/fetch/src/lib.rs); scripts/prepare-native-bundle.py converts a fetched directory into a verified bundle. See docs/native-sdk.md.";

pub fn cmd_update_prepared_manifest(
    aliases: &[String],
    manifest_path: &Path,
    root: &Path,
    store: bool,
    ...
    let mut manifest = if manifest_path.exists() {
        ...
        Manifest {
            schema_version: 1,
            comment: Some(PREPARED_COMMENT.to_string()),
            models: BTreeMap::new(),
            mlx_repos: BTreeMap::new(),
        }
    };
    ...
    for alias in aliases {
        let spec = prepared_source(alias)
            .ok_or_else(|| FetchError::msg(format!("{alias}: not in PREPARED_SOURCES")))?;
        writeln!(
            out,
            "--- pinning {alias}  <-  {}@{} ---",
            spec.repo, spec.revision
        ...
        let mut files = Vec::new();
        for f in spec.files {
            let url = resolve_url(spec.repo, spec.revision, f.remote);
            let target = if store {
                Some(root.join(&dest).join(f.dest))
            } else {
                None
...
mod tests {
    ...
    fn prepared_manifest() -> PathBuf {
        repo_root().join("models/manifests/prepared-sources.json")
    }

    fn hailo_manifest() -> PathBuf {
        repo_root().join("models/manifests/hailo-embeddings.json")
    }

    #[test]
    fn hailo_schema_and_pins() {
        let path = hailo_manifest();
        assert!(
            path.is_file(),
            "missing {}; generate with cargo run -p inferstream-fetch -- --hailo --all --update-manifest --no-store",
            path.display()
        );
        let m = load_manifest(&path).unwrap();
        assert_eq!(m.schema_version, 1);
        let known: HashSet<&str> = HAILO_SOURCES.iter().map(|s| s.alias).collect();
        let have: HashSet<&str> = m.models.keys().map(|s| s.as_str()).collect();
        assert_eq!(
            have, known,
            "hailo-embeddings.json must match HAILO_SOURCES"
        );
        for spec in HAILO_SOURCES {
            let entry = &m.models[spec.alias];
            assert_eq!(entry.repo.as_deref(), Some(spec.repo));
            ...
            for (dest_name, _) in spec.hef_files {
                assert!(
                    dest_names.contains(dest_name),
                    "{} missing {}",
                    spec.alias,
                    dest_name
                );
            ...
            for f in &entry.files {
                assert!(hex64(&f.sha256), "{} {}", spec.alias, f.path);
                assert!(f.size > 0);
                if spec.hf_files.contains(&f.path.as_str()) {
                    // HF files resolve via repo+revision at fetch time.
                    assert!(f.url.is_none(), "{} {}", spec.alias, f.path);
                } else {
                    // HEFs are arch-locked S3 artifacts with absolute URLs.
                    assert!(f.url.is_some(), "{} {}", spec.alias, f.path);
                }
            }
        }
    }
    ...
    #[test]
    fn prepared_schema_and_pins() {
        let path = prepared_manifest();
        assert!(
            path.is_file(),
            "missing {}; generate with cargo run -p inferstream-fetch -- --prepared --all --update-manifest --no-store",
            path.display()
        );
        let m = load_manifest(&path).unwrap();
        assert_eq!(m.schema_version, 1);
        let known: HashSet<&str> = PREPARED_SOURCES.iter().map(|s| s.alias).collect();
        let have: HashSet<&str> = m.models.keys().map(|s| s.as_str()).collect();
        assert_eq!(
            have, known,
            "prepared-sources.json aliases must match PREPARED_SOURCES"
        );
        for spec in PREPARED_SOURCES {
            assert!(hex40(spec.revision), "{} pinned revision", spec.alias);
            assert!(!spec.license.is_empty(), "{} license", spec.alias);
            assert!(
                spec.dest.starts_with("models/prepared-src/"),
                "{} dest",
                spec.alias
            );
            let entry = &m.models[spec.alias];
            ...
            assert_eq!(entry.dest.as_deref(), Some(spec.dest));
            let dest_names: Vec<&str> = entry.files.iter().map(|f| f.path.as_str()).collect();
            // prepare-native-bundle.py requires exactly these staged inputs.
            for required in [
                "model.onnx",
                "tokenizer.json",
                "config.json",
            ...
            for f in spec.files {
                let pinned = entry
                    .files
                    .iter()
                    .find(|e| e.path == f.dest)
                    .unwrap_or_else(|| panic!("{} missing dest file {}", spec.alias, f.dest));
                assert_eq!(
                    pinned.remote_path(),
                    f.remote,
                    "{} {} remote path",
        ...
        // The qualified MiniLM pins recorded in the 2026-09-14 receipt.
        let minilm = &m.models["minilm"];
        assert_eq!(
            minilm.revision.as_deref(),
