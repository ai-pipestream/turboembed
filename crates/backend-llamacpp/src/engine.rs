//! Real llama.cpp engine (crate feature `runtime`).
//!
//! One [`LlamaModel`] per [`LlamaCppBackend`], loaded eagerly at construction
//! so a bad GGUF path or an unsupported device fails at startup. Generation
//! runs on a fresh per-request context inside `spawn_blocking`; decoded token
//! pieces are pushed through a channel onto the response stream as they are
//! produced, so clients see tokens live.
...
use crate::{text_prompt, GenerateParams, LlamaCppConfig, LlamaDevice};
...
impl std::fmt::Debug for LlamaCppBackend {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("LlamaCppBackend")
            .field("model_path", &self.inner.config.model_path)
            .field("device", &self.inner.config.device)
        ...
        let n_gpu_layers = match config.device {
            ...
            LlamaDevice::Cuda => {
                ...
                return Err(BackendError::Unavailable(
                    "device = \"cuda\" but this binary was built without the llama.cpp CUDA \
                     backend; rebuild with --features llamacpp-cuda"
                        .into(),
                ));
                #[cfg(feature = "cuda")]
            ...
            LlamaDevice::Sycl | LlamaDevice::Vulkan => {
                return Err(BackendError::Unavailable(format!(
                    "llama.cpp device {:?} is not compiled into this crate yet \
                     (supported: cpu, cuda, metal)",
...
fn token_piece(model: &LlamaModel, token: LlamaToken, special: bool) -> Result<Vec<u8>, BackendError> {
...
impl Inner {
    ...
    fn generate(
        ...
        let tokens = self
            ...
            .map_err(|e| BackendError::InvalidRequest(format!("prompt tokenization failed: {e}")))?;
        if tokens.is_empty() {
            return Err(BackendError::InvalidRequest("prompt tokenized to nothing".into()));
        }
        ...
        let max_tokens = params.max_tokens.min(n_ctx - tokens.len());
...
impl Backend for LlamaCppBackend {
    ...
    async fn model_metadata(
        ...
        Ok(ModelMetadata {
            ...
            outputs: vec![
                // Streaming: one BYTES piece per chunk; unary: full text.
                TensorMetadata {
                    name: "token".to_string(),
                    datatype: DataType::Bytes.as_oip().to_string(),
                    shape: vec![1],
            ...
            properties: HashMap::from([
                ("model_path".to_string(), inner.config.model_path.clone()),
                ("device".to_string(), inner.config.device.as_str().to_string()),
                ("n_ctx".to_string(), inner.n_ctx.to_string()),
            ]),
        })
    }
    ...
    async fn infer(&self, request: ModelInferRequest) -> Result<ModelInferResponse, BackendError> {
        ...
        let completion = tokio::task::spawn_blocking(move || {
            ...
            Ok::<Vec<u8>, BackendError>(collected)
        })
        .await
    ...
    async fn tokenize(
        ...
        if options.pad_to_longest {
            return Err(BackendError::InvalidRequest(
                "pad_to_longest is not supported by the llama.cpp tokenizer".into(),
            ));
        }
