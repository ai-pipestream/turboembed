...
| `ggml` | SUPPORTED on the RTX 4080 SUPER of `krick` for generation (`compare-ggml-krick-gpu-generate-2026-09-22c.json`, 0.99x of llama.cpp) and GGUF embeddings (`compare-ggml-krick-gpu-embed-2026-09-22b.json`, 0.98x to 1.90x); EXPERIMENTAL on the CPU and on `krickert-mac` (Metal) | every machine; landed on `krick` (RTX 4080 SUPER and CPU) and `krickert-mac` (Apple M2, Metal backend) | llama.cpp through the `llama-cpp-2` binding, built from source with cmake (CUDA backend on `krick`; Metal backend on `krickert-mac`; CPU backend everywhere, including CI) | `ggml_backend_dev` registry; `llama_batch` decode, one token per `step`; a generation owns its own `llama_context` (KV cache) |
