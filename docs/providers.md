...
| `openvino` | SUPPORTED for embeddings on the B70 (`compare-openvino-krick-1-gpu-embed-2026-09-22.json`, 1.15x to 1.55x of the OpenVINO C++ loop); EXPERIMENTAL on CPU (0.91x on four cells) and for the other tasks; NPU listed, not qualified | `krick-1` Battlemage B70 (GPU), any CPU; Intel NPU when available | OpenVINO 2026.3.1 | `ov::Core` compiled model with pooling/normalization/post-processing fused into the graph; `cl_mem` remote tensors on GPU; native WordPiece (`native/wordpiece/`) |
...
| `ggml` | SUPPORTED for generation on the RTX 4080 SUPER of `krick` (`compare-ggml-krick-gpu-generate-2026-09-22.json`, 0.99x of llama.cpp); EXPERIMENTAL for GGUF embeddings (0.94x on two small cells), on the CPU, and on `krickert-mac` (Metal) | every machine; landed on `krick` (RTX 4080 SUPER and CPU) and `krickert-mac` (Apple M2, Metal backend) | llama.cpp through the `llama-cpp-2` binding, built from source with cmake (CUDA backend on `krick`; Metal backend on `krickert-mac`; CPU backend everywhere, including CI) | `ggml_backend_dev` registry; `llama_batch` decode, one token per `step`; a generation owns its own `llama_context` (KV cache) |
