...
| 2 | Infer uses caller USM where the API allows | `ov::InferRequest.set_tensor` wraps the rented pointers (`ov::Tensor(element::i32, {n, seq}, usm)`). Remote OCL `USM_USER_BUFFER` wrap of those ZE pointers is **unavailable** on this stack (`docs/intel-remote-usm-machine-b.md`). `TextEmbeddingPipeline.embed_documents` is **not** on the hot path — that call private-allocs `token_type_ids` and `EmbeddingResults` every embed and has no caller-buffer hook. |
| 3 | Tokenizer write-through (SOLIDIFY 5) | `ov::genai::Tokenizer.encode` has no caller-buffer API. MiniLM uses frozen WordPiece into the rented i32 USM (`docs/tokenizer-write-through.md`). |
