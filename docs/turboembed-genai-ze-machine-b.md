...
| 3 | Tokenizer gap is named, not faked | `ov::genai::Tokenizer.encode` still returns an engine-owned `ov::Tensor` (API has no output buffer). We copy/cast into the rented i32 USM and never pass that encode tensor to infer. |
