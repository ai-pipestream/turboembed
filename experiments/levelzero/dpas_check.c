...
int main(int argc,char**argv){
  ...
  uint16_t *A,*B; float *Cm; C(zeMemAllocShared(ctx,&dd,&hd,8*16*2,64,dev,(void**)&A)); C(zeMemAllocShared(ctx,&dd,&hd,16*16*2,64,dev,(void**)&B)); C(zeMemAllocShared(ctx,&dd,&hd,8*16*4,64,dev,(void**)&Cm));
  float fa[8][16], fb[16][16];
  for(int i=0;i<8;i++)for(int j=0;j<16;j++){fa[i][j]=(float)((i*7+j*3)%11-5); A[i*16+j]=f2h(fa[i][j]);}
  for(int i=0;i<16;i++)for(int j=0;j<16;j++){fb[i][j]=(float)((i*5+j*2)%9-4); B[i*16+j]=f2h(fb[i][j]);}
  ...
  int bad=0; for(int i=0;i<8;i++)for(int j=0;j<16;j++){float s=0; for(int kk=0;kk<16;kk++) s+=fa[i][kk]*fb[kk][j]; if(fabsf(s-Cm[i*16+j])>1e-3){ if(bad<5) printf("C[%d][%d]=%f want %f\n",i,j,Cm[i*16+j],s); bad++;}}
  printf("bad %d\n",bad);
}
