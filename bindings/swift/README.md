...
- `PipestreamTurbo`: the Swift API. `Runtime`, `Context`, `Model`,
  `Session`, and `Result` are final classes owning one C handle each and
  releasing it in `deinit`; a child keeps its parent alive as the C contract
  does. Options are structs mirroring the C descriptors; enums mirror the
  `uint32_t` constants and are checked against the header's values once at
  first use. Every failing call throws `TurboError` with the code, the
  1-based field index, and the library's message.
...
import, and RUN-model binding are reachable through `CTurbo` but have no
Swift wrapper yet (PLAN.md P6 and P7).
