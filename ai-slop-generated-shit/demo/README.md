...
| `java-web-spring/` | a Spring Boot inference server: a JSON REST API under `/api/v1`, the KServe Open Inference Protocol v2 REST surface under `/v2`, an OpenAPI document at `/v3/api-docs`, and a page with the similarity heat map, rerank, tokenize and a summarizer that streams a GGUF model's output as server-sent events | `mvn -f demo/java-web-spring/pom.xml test` (HTTP contract, 65 cases) and `demo/java-web-spring/e2e` (Playwright, 46 cases, browser and both HTTP surfaces) |
...
`/api/v1/generate` and `/api/v1/generate/stream` when started with
`--turbo.generate-bundle=<dir>`. That app serves several bundles at once, so
one server can hold an embedder, a reranker and a generator;
`demo/java-web-spring/README.md` lists every endpoint, the protocol mapping
and the configuration keys, and `demo/java-web-spring/run.sh` shows the flags.
