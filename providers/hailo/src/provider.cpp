...
uint64_t describe_into(Buffer *b, const turbo_buffer_desc &in) {
    ...
    if (bytes == 0) {
        ...
        for (uint32_t i = 0; i < in.ndim; ++i) {
            require(in.shape[i] == 0 || bytes <= UINT64_MAX / in.shape[i], TURBO_E_INVALID_SHAPE,
        ...
        if (take > 0) {
            if (skip == 0) {
                ...
                require(n_text <= scratch.size(), TURBO_E_CAPACITY,
                        "left truncation needs " + std::to_string(n_text) + " staging tokens but the session holds " +
                            std::to_string(scratch.size()));
                size_t n = 0;
    ...
    return boundary(err, [&] {
        ...
        *out = nullptr;
        if (desc != nullptr) {
            ...
            reject_unknown(options_of(desc->options, desc->n_options, "context"), {}, "hailo context");
        }
...
static int32_t x_session_create(void *model, const turbo_session_desc *desc, void **out, turbo_error *err) {
    return boundary(err, [&] {
        ...
        *out = nullptr;
        ...
        require(desc->max_batch >= 1 && desc->max_seq >= 2, TURBO_E_INVALID_ARGUMENT,
    ...
    return boundary(err, [&] {
        ...
        for (uint32_t r = 0; r < batch->batch; ++r) {
            ...
            if (batch->types != nullptr) {
                for (uint32_t c = 0; c < batch->seq; ++c) {
                    // The HEF folds token type 0 into its embeddings; another
                    // type cannot be honored, so it is refused rather than dropped.
                    require(batch->types[src + c] == 0, TURBO_E_UNSUPPORTED,
                            "row " + std::to_string(r) + " column " + std::to_string(c) + " has token_type_id " +
                                std::to_string(batch->types[src + c]) + "; the HEF supports type 0 only");
                }
            }
            std::memcpy(S.ids.data() + dst, batch->ids + src, batch->seq * sizeof(int32_t));
            std::memcpy(S.mask.data() + dst, batch->mask + src, batch->seq * sizeof(int32_t));
...
static int32_t x_session_run(void *s, const turbo_run_options *opts, turbo_provider_result *out, turbo_error *err) {
    return boundary(err, [&] {
        ...
        if (opts != nullptr) {
            ...
            reject_unknown(options_of(opts->params, opts->n_params, "run"), {}, "hailo run");
        }
