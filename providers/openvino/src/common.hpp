...
//
// Shared helpers for the OpenVINO provider: error containment at the plugin
// boundary, string copies into ABI arrays, and the bundle manifest reader.
// The core has already verified bundle hashes before a provider sees a
// bundle directory; the provider re-reads only what it needs.
...
inline int32_t boundary(turbo_error *err, F &&fn) noexcept {
    ...
    try {
        ...
        // OpenVINO and OpenCL exceptions derive from std::exception.
