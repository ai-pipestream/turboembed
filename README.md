...
- **FINAL SOLIDIFY bench (Machine C)** — **LIVE.** `make bench-machine-c` writes `testdata/receipts/bench/machine-c-metal.json`. Gates: measured p50/p99, `allocs/forward == 0`, Berlin + embed goldens in band, Metal SHARED, no CPU fallback (`docs/apple-solidify-bench-machine-c.md`).
